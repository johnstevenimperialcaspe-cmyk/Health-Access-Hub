...
import StudAccount from "./StudAccount";
...
const useAuthGuard = () => {
  ...
  return {
    ready: !loading && !isInitialising && currentUser?.role === "student",
    user: currentUser,
    logout, // expose logout for use in component
  };
};

// MAIN COMPONENT
const Student = () => {
  // 1. Auth guard + navigate
  const { ready, user, logout } = useAuthGuard();
  const navigate = useNavigate(); // Now defined here

  // 2. Section state (persisted in localStorage)
  const [currentSection, setCurrentSection] = useState(() => {
    return localStorage.getItem("studentCurrentSection") || "dashboard";
  });
  ...
  // 3. Persist section on change
  useEffect(() => {
    if (ready) {
      localStorage.setItem("studentCurrentSection", currentSection);
    }
  ...
  // 5c. Pre-appointment assessment modal state
  const [assessmentModalVisible, setAssessmentModalVisible] = useState(false);

  // 5a. Appointment slot availability state
  ...
  // 7. Load data from APIs when ready
  useEffect(() => {
    if (!ready) return;
    Promise.all([
      loadAppointments(),
  ...
  // 8. Compute dashboard data from current state when ready or when data changes
  useEffect(() => {
    if (!ready) return;
  ...
  // FIXED: Now `navigate` is in scope
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        >
          ...
          >
            ...
            {/* PROFILE */}
            {currentSection === "account" && <StudAccount currentUser={user} />}
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <FormControl fullWidth required>
              ...
              >
                ...
                <MenuItem value="Medical Certificate Request">Medical Certificate Request</MenuItem>
      ...
      {/* LOGOUT MODAL */}
      <Modal
        isOpen={modalOpen && modalType === "logout"}
