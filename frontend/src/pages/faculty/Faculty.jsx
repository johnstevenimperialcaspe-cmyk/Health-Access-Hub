...
const useAuthGuard = () => {
  ...
  useEffect(() => {
    if (loading || isInitialising) {
      console.log("🔐 Faculty Auth Guard: Still loading/initialising...");
      return;
    }

    console.log("🔐 Faculty Auth Guard: Auth ready. currentUser:", currentUser);

    if (!currentUser) {
      console.log("🔐 Faculty Auth Guard: No currentUser, redirecting to login");
      navigate("/login", { replace: true });
      return;
    }

    // Support faculty role
    const isAllowed = currentUser.role === "faculty";
    console.log("🔐 Faculty Auth Guard: User role:", currentUser.role);
    console.log("🔐 Faculty Auth Guard: Is allowed?", isAllowed);
    
    if (!isAllowed) {
      console.log("🔐 Faculty Auth Guard: Access denied for role:", currentUser.role);
  ...
  return {
    ready: !loading && !isInitialising && currentUser && currentUser.role === "faculty",
    user: currentUser,
    logout,
  };
};
...
const Faculty = () => {
  ...
  const [currentSection, setCurrentSection] = useState(() => {
    return localStorage.getItem("facultyCurrentSection") || "dashboard";
  });
  ...
  // Show spinner while auth resolves or user data is loading
        ...
        >
          ...
          >
            ...
            {currentSection === "dashboard" && (
              <Dashboard
                data={dashboardData}
                facultyName={user ? `${user.first_name || ""} ${user.last_name || ""}`.trim() || "Faculty" : "Faculty"}
                facultyId={user?.employee_id || user?.faculty_id || "N/A"}
                onScheduleAppointment={() => setAssessmentModalVisible(true)}
        ...
        <form onSubmit={handleAppointmentSubmit}>
          <Box sx={{ mb: 3 }}>
            ...
            <TextField
              ...
              name="faculty_id"
              fullWidth
              value={user.employee_id || user.faculty_id || ""}
              disabled
