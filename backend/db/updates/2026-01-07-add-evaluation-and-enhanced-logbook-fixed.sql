...
-- Note: Using separate statements to handle existing columns gracefully
-- ==========================================
...
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE TABLE_SCHEMA=DATABASE()
     ...
     AND COLUMN_NAME='user_role') > 0,
  'SELECT 1',
  'ALTER TABLE `audit_logs` ADD COLUMN `user_role` VARCHAR(50) DEFAULT NULL AFTER `actor_id`'
));
...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT 1',
  'ALTER TABLE `audit_logs` ADD COLUMN `details` TEXT DEFAULT NULL AFTER `metadata`'
));
PREPARE alterStatement FROM @preparedStatement;
EXECUTE alterStatement;
DEALLOCATE PREPARE alterStatement;

-- Add indexes (these will silently skip if they already exist in newer MySQL versions)
